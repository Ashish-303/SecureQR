"""
MODEL 3: SUPPORT VECTOR MACHINE (LINEAR SVC) - IMPROVED
...
print("=" * 80)
print("MODEL 3: SUPPORT VECTOR MACHINE (LINEAR SVC) - IMPROVED")
print("=" * 80)
...
MODEL_PATH = os.path.join(BASE_DIR, "..", "models", "model3_svm.pkl")
VECTORIZER_PATH = os.path.join(BASE_DIR, "..", "models", "model3_vectorizer.pkl")
...
# ✅ KEY CHANGE: Use 90% of ALL available data instead of hard undersampling
# This keeps far more samples while still maintaining balance
benign_count = len(benign)
malicious_count = len(malicious)
min_count = min(benign_count, malicious_count)

# Use 90% of minority class (much more data than before)
sample_size = int(min_count * 0.90)
print(f"   ✓ Sampling {sample_size:,} from each class (90% of minority class)...")

benign_sample = benign.sample(n=sample_size, random_state=42)
malicious_sample = malicious.sample(n=sample_size, random_state=42)
...
vectorizer = TfidfVectorizer(
    ...
    ngram_range=(2, 6),       # ✅ Wider range: 2-6 (was 3-5) captures more patterns
    max_features=50000,       # ✅ Increased: 50k features (was 10k)
    min_df=2,                 # ✅ Lower min_df to keep more rare but useful patterns
    max_df=0.95,              # Slightly relaxed max_df
...
# ================================
# SPLIT DATA (85/15 for more training data)
# ================================

print("\n[STEP 7/9] Splitting dataset (85% train, 15% test)...")
X_train, X_test, y_train, y_test = train_test_split(
    X_vectorized,
    y,
    test_size=0.15,           # ✅ Changed from 0.20 to 0.15 → more training data
    random_state=42,
    stratify=y
)
...
print("\n[STEP 8/9] Training Support Vector Machine (Linear SVC)...")
print("   This may take a few minutes...")

# ✅ Wrapped with CalibratedClassifierCV so predict_proba works in predictor.py
svm = LinearSVC(
    C=0.5,                      # ✅ Slightly lower C to reduce overfitting
...
# CalibratedClassifierCV adds predict_proba support to LinearSVC
...
print("=" * 80)
print(f"Model: Support Vector Machine (Linear SVC) - Improved")
print(f"\nTRAIN SET:")
